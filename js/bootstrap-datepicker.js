...
(function ($, undefined) {
	...
	Datepicker.prototype = {
		...
		update: function () {
			...
			let oldDates = this.dates.copy(),
				...
				fromArgs = false;
			if (arguments.length) {
				$.each(arguments, $.proxy(function (_i, date) {
		...
		fill: function () {
			...
			$.each(this.dates, function (_i, e) {
				if (e.getUTCFullYear() === year)
					months.eq(e.getUTCMonth()).addClass('active');
			});
			...
			let yearCont = this.picker.find('.datepicker-years')
				...
				.find('td');
			year -= 1;
			let years = $.map(this.dates, function (e) {
	...
	function opts_from_locale(lang) {
		...
		let d = dates[lang];
		$.each(locale_opts, function (_i, k) {
			if (k in d)
				out[k] = d[k];
		});
		return out;
	...
	let DPGlobal = {
		...
		parseDate: function (date, format, language) {
			...
			if (parts.length !== fparts.length) {
				fparts = $(fparts).filter(function (_i, p) {
